                ...
                <div className="flex flex-col md:flex-row w-full md:items-center gap-4 order-2 md:order-1 z-10">
                    {navLinks.map((item) => (
                        <Link 
                            key={item.name}
                            href={item.link}
                            className={`text-sm ${ currentPage === item.link && 'font-semibold'} px-5 py-2 md:px-2 md:py-0 bg-cardBg md:bg-transparent rounded-full border-paragraphBlack/40 md:border-none border-[0.5px] hover:bg-secondary ${navActive ? 'opacity-100' : 'opacity-0'}`}
                        >
                            {item.name}
                        </Link>
                    ))}
